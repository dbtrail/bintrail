...
An example that provisions an EC2 instance to run the indexer, with credentials stored in AWS Secrets Manager and user data that installs bintrail and the systemd timers on first boot.

**`variables.tf`**
```hcl
variable "index_dsn" {
  description = "DSN for the bintrail index MySQL database"
  type        = string
  sensitive   = true
}

variable "source_dsn" {
  description = "DSN for the source MySQL server"
  type        = string
  sensitive   = true
}

variable "binlog_dir" {
  description = "Path to MySQL binlog directory on the indexer host"
  type        = string
  default     = "/var/lib/mysql"
}

variable "retain" {
  description = "Partition retention period (e.g. 7d, 30d)"
  type        = string
  default     = "7d"
}
...
**`main.tf`**
```hcl
resource "aws_secretsmanager_secret" "bintrail" {
  name                    = "bintrail/dsn"
  recovery_window_in_days = 0
}

resource "aws_secretsmanager_secret_version" "bintrail" {
  secret_id = aws_secretsmanager_secret.bintrail.id
  secret_string = jsonencode({
    index_dsn  = var.index_dsn
    source_dsn = var.source_dsn
  })
}

resource "aws_iam_role" "bintrail" {
  name               = "bintrail-indexer"
...
resource "aws_iam_role_policy" "bintrail_secrets" {
  role = aws_iam_role.bintrail.id
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Effect   = "Allow"
      Action   = ["secretsmanager:GetSecretValue"]
      Resource = aws_secretsmanager_secret.bintrail.arn
    }]
  })
}

resource "aws_iam_instance_profile" "bintrail" {
  name = "bintrail-indexer"
  role = aws_iam_role.bintrail.name
}

resource "aws_instance" "bintrail" {
  ami                  = data.aws_ami.amazon_linux.id
  instance_type        = var.instance_type
  iam_instance_profile = aws_iam_instance_profile.bintrail.name

  user_data = templatefile("${path.module}/user_data.sh.tftpl", {
    secret_arn = aws_secretsmanager_secret.bintrail.arn
    binlog_dir = var.binlog_dir
    retain     = var.retain
  })
...
# Install bintrail
curl -fsSL https://github.com/bintrail/bintrail/releases/latest/download/bintrail-linux-amd64 \
  -o /usr/local/bin/bintrail
chmod +x /usr/local/bin/bintrail
...
INDEX_DSN=$(echo "$SECRET"  | python3 -c "import sys,json; print(json.load(sys.stdin)['index_dsn'])")
SOURCE_DSN=$(echo "$SECRET" | python3 -c "import sys,json; print(json.load(sys.stdin)['source_dsn'])")

install -d -m 750 /etc/bintrail
printf 'INDEX_DSN=%s\nSOURCE_DSN=%s\n' "$INDEX_DSN" "$SOURCE_DSN" \
  > /etc/bintrail/env
chmod 640 /etc/bintrail/env

# Initialise the index (idempotent)
bintrail init --index-dsn "$INDEX_DSN"

# Install systemd units (copy from the examples in the README)
cat > /etc/systemd/system/bintrail-index.service <<'EOF'
[Unit]
Description=Bintrail binlog indexer
After=network.target
...
EnvironmentFile=/etc/bintrail/env
ExecStart=/usr/local/bin/bintrail index \
  --index-dsn ${INDEX_DSN} --source-dsn ${SOURCE_DSN} \
  --binlog-dir ${binlog_dir} --all
EOF

cat > /etc/systemd/system/bintrail-index.timer <<'EOF'
[Unit]
Description=Run bintrail indexer every 5 minutes
[Timer]
OnBootSec=1min
...
cat > /etc/systemd/system/bintrail-rotate.service <<'EOF'
[Unit]
Description=Bintrail partition rotation
After=network.target
...
EnvironmentFile=/etc/bintrail/env
ExecStart=/usr/local/bin/bintrail rotate \
  --index-dsn ${INDEX_DSN} --retain ${retain} --add-future 2
EOF

cat > /etc/systemd/system/bintrail-rotate.timer <<'EOF'
[Unit]
Description=Run bintrail partition rotation nightly
[Timer]
OnCalendar=*-*-* 01:00:00
...
```sh
terraform init
terraform plan -var="index_dsn=user:pass@tcp(host:3306)/binlog_index" \
               -var="source_dsn=user:pass@tcp(source:3306)/"
terraform apply
```

> Use a `terraform.tfvars` file or a secrets backend (e.g. AWS Secrets Manager, HashiCorp Vault, 1Password) to avoid passing DSNs on the command line.

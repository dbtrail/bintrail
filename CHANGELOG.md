...
### Fixed
- Full-table reconstruct now correctly handles **DATETIME/TIMESTAMP primary keys**. The initial #187 implementation used `fmt.Sprintf("%v", ...)` to hash PK values on both sides of the merge, but the bintrail indexer stores DATETIME pk_values as go-mysql-formatted strings (`"2026-04-11 14:30:45"`) while DuckDB's `parquet_scan` returns the same column as a `time.Time`. The resulting `%v` strings diverged and every event for a DATETIME-PK table silently missed the baseline, producing a dump with duplicate PK rows. New `canonicalizePKValue` helper in `internal/reconstruct` normalises DuckDB scan types back to the indexer's format before hashing. New integration test `TestRunReconstruct_fullTableRoundTrip_datetimePK` pins the contract — it fails loudly with a duplicate-key error when the canonicalizer is removed, which is how the fix was validated. Also added `TestRunReconstruct_fullTableRoundTrip_varcharPK` to pin VARCHAR PK behavior. The #187 v1 caveat about DATETIME/TIMESTAMP PKs is now removed; DECIMAL, BINARY, BLOB, BIT, and JSON PKs remain unsupported and now emit an `slog.Warn` at reconstruct-start via the new `supportedPKType` guard (#212).

...
### Added
- `bintrail-mcp-gateway` accepts a per-tenant `auth_secret` on the OAuth authorize page so guessing a tenant ID alone is no longer sufficient to obtain an access token. The secret is bcrypt-hashed (`bcrypt.DefaultCost`) before persistence; the cleartext is never echoed on a GET tenant response or in admin/oauth logs. Admin endpoints (`POST /admin/tenants`, `PUT /admin/tenants/:id`) accept the cleartext via the `auth_secret` JSON field; a PUT that omits the field preserves the existing hash (no silent downgrade). The authorize HTML form gained a "Tenant Secret" password input alongside the existing "Tenant ID" field, and `POST /oauth/authorize` validates the submission via `Tenant.VerifySecret` before issuing an authorization code. Backward compatibility: tenants created before this PR have an empty `AuthSecretHash` and continue to authorize without a secret (gradual rollout), but emit `slog.Warn("authorize accepted on legacy tenant without auth_secret; …")` on every authorize so operators can grep gateway logs for the inventory of unmigrated tenants. New direct dependency: `golang.org/x/crypto/bcrypt` (#132).

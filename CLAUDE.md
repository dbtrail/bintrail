...
| `proxysql-config` | `proxysql_config.go` | `--out` (default `proxysql-setup.sql`, `-` for stdout), `--shim-config` (default `shim.yaml`), `--mysql-port` (default `3306`), `--shim-port` (default `3308`), `--proxysql-mysql-port` (default `6033`); reads `BINTRAIL_SOURCE_DSN` and shim.yaml; outputs SQL using hostgroups 990/991 and rule_ids 990001-990004 (the fourth rule matches the `/*+ DBTRAIL_AT='<ts>' */` hint-comment form) |

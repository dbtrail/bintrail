...
| `dump` | `dump.go` | `--source-dsn` (req), `--output-dir` (req), `--schemas`, `--tables`, `--mydumper-path` (default `mydumper`), `--mydumper-image` (default `mydumper/mydumper:latest`), `--threads` (default 4), `--encrypt`, `--encrypt-key`, `--format` |
| `baseline` | `baseline.go` | `--input` (req), `--output` (req), `--timestamp`, `--tables`, `--compression` (default `zstd`), `--row-group-size` (default 500000), `--upload` (S3 URL), `--upload-region`, `--encrypt`, `--encrypt-key`, `--format` |
| `generate-key` | `generate_key.go` | `--output` (default `~/.config/bintrail/dump.key`), `--format` |
| `upload` | `upload.go` | `--source` (req), `--destination` (req), `--region`, `--retry`, `--index-dsn`, `--format` |

Flag variable naming convention: prefixed by command abbreviation (e.g. `idxIndexDSN`, `qSchema`, `rDryRun`, `rotRetain`, `rotArchiveDir`, `rotArchiveCompression`, `stIndexDSN`, `strmIndexDSN`, `dmpSourceDSN`, `dmpEncrypt`, `bslInput`, `bslEncrypt`, `gkOutput`, `uplSource`).
...
- `--encrypt` → `--exec-per-thread "openssl enc -aes-256-cbc -pbkdf2 -pass file:<key>"` + `--exec-per-thread-extension ".enc"`

**Encryption at rest** (`--encrypt`, `--encrypt-key`): mydumper has no native encryption. Instead, `--exec-per-thread` pipes each output file through OpenSSL AES-256-CBC. The key file path defaults to `~/.config/bintrail/dump.key` (generated by `bintrail generate-key`). `resolveEncryptKey` validates the key file exists and `openssl` is on `$PATH`. In Docker mode, `buildDockerArgs` bind-mounts the key file read-only.
...
**Encrypted dump support** (`--encrypt`, `--encrypt-key`): when `--encrypt` is set, `decryptDumpFiles` walks the input directory and decrypts every `.enc` file using `openssl enc -d -aes-256-cbc -pbkdf2`. Decrypted files are written alongside (stripping `.enc`), then `DiscoverTables` finds them normally. A cleanup function removes decrypted files after `baseline.Run` completes.

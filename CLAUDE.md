...
internal/
  ...
  duckdbutil/          # Shared DuckDB session helpers: EnableS3CredentialChain (aws-extension credential_chain secret, best-effort — gives the httpfs baseline-read paths SDK-chain parity, #459)
...
| `recover` | `recover.go` | same event filters as query (schema/table/pk/event-type/gtid/since/until/changed-column/flag) + `--output`, `--dry-run`, `--limit` (default 1000), `--profile`, `--no-archive` — but NO `--archive-s3`/`--bintrail-id`: archive access on recover is exclusively `archive_state` auto-discovery (those explicit flags exist on `query` only) |

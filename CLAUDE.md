...
| `shim` | `shim.go` | `--listen` (default `127.0.0.1:3308`), `--index-dsn` (req), `--shim-config` (default `shim.yaml`), `--no-archive`; in-process MySQL-protocol server for `_flashback`/`_diff`/`_snapshot` virtual schemas, sits behind ProxySQL. **`shim.yaml` requires `mysql_password` (cleartext) per tenant from 0.7.2 on**; `mysql_pass_sha1` is parsed (UnmarshalStrict compat) but rejected with a migration error. The shim validates the cleartext via `mysql_native_password`; `proxysql-config` recomputes the SHA1 for ProxySQL's `mysql_users.password`. |

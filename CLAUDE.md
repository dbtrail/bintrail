...
docs/                  # guide.md, indexing.md, query-and-recovery.md, streaming.md, rotation-and-status.md, mcp-server.md, upload.md, parquet-debugging.md, deployment.md, quickstart.md, dump-and-baseline.md, docker.md, server-identity.md, mcp-gateway.md, console.md
```
...
| `recover` | `recover.go` | shared event filters with query: schema/table/pk/`--pks`/`--limit-per-pk`/event-type/gtid/since/until/`--column-eq`/flag (`--changed-column` is query-ONLY) + `--output`, `--dry-run`, `--limit` (default 1000), `--profile`, `--no-archive` — but NO `--archive-s3`/`--bintrail-id`: archive access on recover is exclusively `archive_state` auto-discovery (those explicit flags exist on `query`, not `recover`) |

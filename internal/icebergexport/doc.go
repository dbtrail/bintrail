...
//     cliapp/icebergfree_test.go derives the guarded set from `go list ./...`
//     minus a short allowlist, so a new package is guarded by default, and the
//     console, MCP and pg binaries must never link either library.
...
// equality-delete file naming every touched key plus a data file with the
// after-image of every key that still exists. Iceberg applies an equality

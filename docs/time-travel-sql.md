...
- **`_snapshot` is baseline-aware; `_flashback` is binlog-only.** Start `bintrail shim` with `--baseline-dir <dir>` or `--baseline-s3 s3://bucket/prefix/` (the snapshots produced by `bintrail baseline`) to enable it. A **single-row** `_snapshot` query (`WHERE <pk> = <value>`) then seeds the row state from the baseline at-or-before the AS OF instant and applies post-snapshot binlog events on top — so a row that existed at AS OF but was never touched within the retained binlog window still resolves. `_flashback` deliberately stays binlog-only, so the two schemas have distinct, observable semantics. With no baseline source configured, `_snapshot` degrades to the binlog-only `_flashback` behaviour (a `Debug` log notes the fallback). Caveats: the baseline lookup applies to the single-row shape only — **full-table `_snapshot`** still resolves binlog-only for now (tracked as a follow-up). The single-row baseline match is supported for **integer, `YEAR`, `DECIMAL`/`NUMERIC`, string (`CHAR`/`VARCHAR`/`TEXT`/`ENUM`/`SET`), and `DATETIME`/`TIMESTAMP`/`DATE` PKs** (the shim pins the DuckDB session to UTC so temporal-PK matches resolve deterministically on any host timezone). **`FLOAT`/`DOUBLE`, `BINARY`/`VARBINARY`/`BLOB`, `BIT`, `JSON`, and spatial PK types fall back to binlog-only with a `Warn`** (their Parquet representation doesn't match reliably against a string literal). For full-table point-in-time reconstruction, or untouched rows keyed by an unsupported PK type, use the offline `bintrail reconstruct` command.

...
- **ENUM/SET labels are decoded with the latest schema snapshot.** Binlog row images store ENUMs as ordinals and SETs as bitmasks; the shim maps them back to labels using the *latest* snapshot's column definition. Appending members (the common evolution) is safe, and an ordinal beyond the current definition is returned as the raw number — but if members were **reordered or removed from the middle** between an event and the latest snapshot, ordinals from before that ALTER decode to the wrong label with no warning. If you reshape an ENUM in place, treat pre-ALTER time-travel labels for that column with suspicion (the raw ordinal is recoverable from `bintrail query`'s JSON output, which is deliberately left unmapped).

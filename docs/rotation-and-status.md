...
If rotation fails — or keeps deferring everything to a stalled archiving flow — for 3 consecutive cycles, the loop escalates to an explicit Error in the logs: the index is growing unbounded and needs attention. The explicit `bintrail rotate` command is unaffected by all of the above: it keeps its unguarded, operator-asked-for-it semantics.

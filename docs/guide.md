...
| `column count mismatch — skipping` / `table not in snapshot — skipping` (logged as a warning) | Table schema changed since last snapshot | Re-run `bintrail snapshot`. Warn-and-skip only applies to `stream`, system schemas (`mysql`, `sys`, `information_schema`, `performance_schema`), and events older than the snapshot — for anything else, file-mode `bintrail index` fails loud instead (next row). |

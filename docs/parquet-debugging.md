...
- **An S3-compatible store is named in the file.** When the generating process runs with `BINTRAIL_S3_ENDPOINT`, the secret carries `ENDPOINT`, `URL_STYLE` and `USE_SSL`, so DuckDB on another machine reads the same store instead of AWS. A location, not a credential: the file stays shareable.

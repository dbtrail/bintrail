...
- **A table with no changes is not rewritten, if you ask for it.** Off by default (`--baseline-carry-forward-unchanged`, env `BINTRAIL_BASELINE_CARRY_FORWARD_UNCHANGED`, or the **Automatic backup refresh** card under Settings, which overrides the flag and applies on the next cycle without a restart). When the delta window held no events for a table, its previous Parquet file is published into the new snapshot as-is (a hard link where the filesystem allows one, a copy otherwise) instead of being folded and re-emitted. That is correct rather than a shortcut: each table's file carries its OWN binlog anchor, so an untouched table's anchor still points exactly where its deltas resume. It is opt-in because the rows are identical either way but the on-disk representation is not: a hard link makes two snapshots share one file, so `du` and a prune report space they will not reclaim while the newer snapshot references it, and one snapshot then holds tables anchored at different binlog coordinates. The run summary calls it `unchanged` rather than `refreshed`, because which tables actually cost a full rewrite is the number worth seeing. A `TRUNCATE`, `DROP` or `RENAME` emits no row events either, which is why the destructive-DDL refusal runs first and this can never republish rows a truncate deleted.

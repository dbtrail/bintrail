...
Minutes matter because this interval is, in practice, the freshness of reporting. The generated `state_<schema>_<table>` views read the newest snapshot, so a query run against the Parquet tier sees the world as of the last refresh. At `12h` that is a tier for yesterday's questions; at `15m` it is a tier for this morning's. A malformed interval refuses at startup; a daemon with nothing refreshable yet starts and says so, because every tick re-checks — servers added from the console later are picked up automatically, and refusing would mean a compose file carrying the interval could not boot a fresh install.
...
- **Nothing is uploaded, so nothing is pruned.** A refreshed snapshot is written locally only, and `bintrail baseline prune` reclaims a snapshot only once it has confirmed a durable S3 copy of it — so snapshots this loop publishes accumulate, whether or not an S3 destination is configured. Upload and prune them on your own schedule (`bintrail upload --source <baseline-dir> --destination s3://…`), or size the disk for the rate the interval implies. **Read that rate before choosing a short interval**: it is one full-table snapshot per monitored server per tick, so `1h` is 720 a month per server and `5m` is 8,640, none of them reclaimable. The startup warning prints that projection for the interval you set, over 30 days because that is the horizon a disk fills on, rather than leaving the arithmetic to be discovered from a full disk.
- **A table with no changes can be left as it is (opt-in, off by default).** Turn it on with `--carry-forward-unchanged` on `bintrail baseline refresh`, or for the daemon with `--baseline-carry-forward-unchanged`, env `BINTRAIL_BASELINE_CARRY_FORWARD_UNCHANGED` (a true/false value: `1`, `true`, `0`, `false`), or the **Automatic backup refresh** card under Settings -> Storage, which overrides the flag and applies on the next cycle without a restart. That card also has a **Use the daemon setting** button, which clears the saved value and hands the decision back to the flag. When the delta window held no events for a table, its previous Parquet file is published into the new snapshot as-is (a hard link where the filesystem allows one, a copy otherwise) instead of being folded and re-emitted. That is correct rather than a shortcut: each table's file carries its OWN binlog anchor, so an untouched table's anchor still points exactly where its deltas resume, which is also why a snapshot holding tables anchored at different coordinates is not the mixed snapshot the all-or-nothing rule above refuses. That rule is about a table published at the wrong instant; a carried table is published at exactly its own. It is opt-in because the rows are identical either way but the on-disk representation is not: a hard link makes two snapshots share one file, so `bintrail baseline prune` reports space it will not reclaim while the newer snapshot references it (a `du` run per snapshot directory double-counts it too; one `du` over the baseline root reports the truth). The run summary calls a reused table `unchanged` rather than `refreshed`, and the console's refresh line names how many were reused, because which tables actually cost a full rewrite is the number worth seeing. A `TRUNCATE`, `DROP` or `RENAME` emits no row events either, which is why the destructive-DDL refusal runs first and this can never republish rows a truncate deleted.

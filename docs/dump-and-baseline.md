...
**From the console, per server.** The Backups page can set a schedule for one server without any daemon flag (#1442): every N minutes/hours/days (at least `15m`), lined up on a UTC time of day, running either a full backup (the Create backup job, needs the creation opt-in) or this same rebuild from the change history. It lives on the server's registry entry as `backup_schedule` and the watch daemon reads it every minute, so it applies without a restart. Three rules keep it predictable: the timetable is a fixed grid (`every 1d at 03:00` is 03:00 UTC daily, `every 6h at 03:00` is 03/09/15/21), a time that passed while the daemon was stopped is not made up, and a scheduled run that finds another backup job on the server skips that time rather than queuing. Every scheduled run and every skip lands in the baseline run history, so the page shows the last run, its result and the last skip, restart or not. A per-server schedule and the daemon-wide interval can run side by side; they share the one-job-per-server lock like everything else that produces a backup.

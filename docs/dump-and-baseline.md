...
- **Isolated from capture.** A failure is logged and retried; it can never take down the stream or the supervisor. That includes an internal error inside the refresh itself, not only one it reports: the run is marked failed with its stack in the daemon log, and the other backup jobs that share this server's lock (a manual backup, a restore, a custom `.sql` build) stay available. A baseline that stopped refreshing is a degradation, a daemon that stopped capturing is an outage, and the first must never cause the second.

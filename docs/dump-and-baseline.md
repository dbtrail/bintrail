...
- **A table with no changes is not rewritten.** When the delta window held no events for a table, its previous Parquet file is published into the new snapshot as-is (a hard link where the filesystem allows one, a copy otherwise) instead of being folded and re-emitted. That is correct rather than a shortcut: each table's file carries its OWN binlog anchor, so an untouched table's anchor still points exactly where its deltas resume. The run summary calls it `unchanged` rather than `refreshed`, because which tables actually cost a full rewrite is the number worth seeing. A `TRUNCATE`, `DROP` or `RENAME` emits no row events either, which is why the destructive-DDL refusal runs first and this can never republish rows a truncate deleted.
- **An interval shorter than a refresh is a request, not a schedule.** Every refresh rewrites each table in full, however little of it changed, so it has a cost the interval cannot go below. Asking for less does not queue refreshes up: a server whose previous refresh is still folding is skipped for that tick, and the tick says so. Each refresh also logs its own duration, and one that outran the configured interval says so explicitly, naming the server. That duration is the honest measure of what a full rewrite costs on your data, which is the number to size a shorter interval against.

...
| `--lock-mode` | How mydumper syncs its threads onto one instant: `ftwrl` (default, point-consistent), `safe-no-lock` (no extra privilege, aborts rather than emit a torn snapshot), `no-lock` (accepts a torn snapshot) | `ftwrl` |

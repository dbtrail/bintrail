...
Typing these columns requires a schema snapshot that describes the table. If no snapshot is loaded **at all**, or the loaded snapshot doesn't cover the event's table, `recover` **refuses** the event rather than risk writing a base64 string verbatim into a real column — take a snapshot (`bintrail snapshot`) covering the table and retry. This refusal applies to every real `recover` run (which always reaches the index over a DSN); the only path that still degrades to schemaless, all-columns mode is a direct library caller that constructs the generator with no database handle and hand-feeds its own rows.

...
That strictness deliberately includes stale registrations: an `archive_state` row pointing at a local path whose Parquet files were later deleted (or never written) fails the strict query — the planner counted those hours as covered *from `archive_state`*, so a source that can't be read can't be proven harmless. The remediation is to delete the stale `archive_state` rows (or restore the files); `--allow-gaps` degrades to warn-and-continue if you accept a possibly-incomplete result. The same applies to a transiently unreachable S3 source (expired credentials, throttling): strict mode prefers a clear error over a silently incomplete reconstruction.

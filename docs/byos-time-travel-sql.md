...
The generated file looks like this (with your real values in the `'...'` slots):

```yaml
# Bintrail BYOS time-travel SQL — shim configuration
# Generated by bintrail init-shim. See docs/byos-time-travel-sql.md.
#
# Fill in mysql_user and mysql_pass_sha1 below — these are the
# credentials ProxySQL will use to authenticate your application.

listen: ':3308'

tenants:
  - server_id: '99999'
    source_dsn: 'app_user:...@tcp(127.0.0.1:3306)/myapp'
    agent_url: 'http://localhost:8600'
    agent_token: 'btk_...'
    # TODO: fill in your application's MySQL credentials
    # mysql_user: app_user
    # TODO: SHA1 hex of mysql_user's password
    # See https://proxysql.com/documentation/users/
    # mysql_pass_sha1:
```

You need to fill in **`mysql_user`** and **`mysql_pass_sha1`**. The username is whatever you want your application to use. The SHA1 is in ProxySQL's `*HEX` format — generate it with MySQL:
...
Verify ProxySQL accepted the config:

```sh
mysql -u admin -p -h 127.0.0.1 -P 6032 -e "SELECT hostgroup_id, hostname, port FROM runtime_mysql_servers WHERE hostgroup_id IN (990,991);"
# +--------------+-----------+------+
...
The shim should report `listening on :3308` once it has loaded `shim.yaml` and connected to the agent at `http://localhost:8600`.
...
Either the row didn't exist at that timestamp, or the requested time falls in a gap between the agent's in-memory buffer and the S3 archive. Check what the agent has:

```sh
journalctl -u bintrail-agent | grep -i "buffer\|s3 flush" | tail
```

The buffer retains the last `--buffer-retain` (default `6h`) and S3 has everything older that has been flushed.

### Agent unreachable from the shim

If `dbtrail-shim` log shows `connection refused: localhost:8600`, the bintrail agent isn't running or isn't listening on its default HTTP port. Verify:

```sh
systemctl status bintrail-agent
curl -sf http://localhost:8600/healthz && echo OK
```

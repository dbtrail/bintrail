...
| `GET /api/activity` | Window aggregate behind the Overview tiles: counts by event type, distinct tables touched, and a per-table breakdown over a **stated period**. `?period=` accepts `1h`, `6h`, `24h` (default `24h`); anything else is a `400`. Returns `{period, label, since, until, total, inserts, updates, deletes, other, tables, top_tables, complete, notes}`. `complete: false` means part of the window is not in these counts — most often hours already archived to Parquet, which this endpoint does not scan — and `notes` says what; the UI then marks the affected tiles "partial". The period allowlist is capped at 24 h on purpose: `binlog_events` is hour-partitioned, so a bounded `event_timestamp` range prunes to the window's partitions, but no index leads with `event_timestamp` — a week-long aggregate is a reporting query, not a dashboard tile. RBAC deny rules are applied, so a denied table contributes to neither the counts nor `top_tables`. |
